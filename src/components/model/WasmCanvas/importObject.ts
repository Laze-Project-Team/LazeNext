...
import type { getCompleteImportsFunction, importObject, WasmImportProps } from '@/typings/compiler';

import { strToMem } from './utilities/strToMem';

export const getImports = (props: WasmImportProps, dispatcher?: Dispatch): getCompleteImportsFunction => {
  ...
  const imports = {
    ...
    js: {
      ...
      updateLinetraceTime: (time: number) => {
        time.toPrecision(4);
        props.variables.linetraceTime = time;
      },
      lockPointer: () => {
    ...
    arduino: {
      setUp: async (vendorId: bigint, index: number) => {
        const executeCallback = () => {
          if (props.variables.lazeCallNoParam) {
            props.variables.lazeCallNoParam(index);
          }
        };
